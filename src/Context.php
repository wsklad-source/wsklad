...
final class Context
{
	...
	{
		do_action('wsklad_context_loaded');
	}
	...
	{
		if(false === isset($_GET['wsklad-receiver']))
		{
			return false;
		}

		if(wsklad()->getVar($_GET['wsklad-receiver'], false))
		{
			return true;
		}
	...
	public function isNetworkActive()
	{
		...
		if(null === $this->network_active)
		{
			if(is_multisite())
			{
				...
				$this->network_active = strpos(wp_normalize_path(__FILE__), wp_normalize_path(WPMU_PLUGIN_DIR) ) === 0 || in_array(WP_PLUGIN_DIR . '/' . wsklad()->environment()->get('plugin_basename'), $network_active_plugins, true);
			}
			else
			{
