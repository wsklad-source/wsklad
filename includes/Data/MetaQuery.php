...
class MetaQuery
{
	...
	{
		return wsklad_wpdb()->base_prefix . 'wsklad_' . $type . 's_meta';
	}
	...
	public function get_sql($type, $primary_table, $primary_id_column, $context = null)
	{
		...
		(WSKLAD_PREFIX . 'data_meta_get_sql',
			[
				$sql,
				$this->queries,
				$type,
	...
	public function get_sql_for_clause(&$clause, $parent_query, $clause_key = '')
	{
		...
		if(!$clause_key || is_int($clause_key))
		{
		...
		if(array_key_exists('value', $clause))
		{
			...
			if($where)
			{
				if('CHAR' === $meta_type)
				{
					$sql_chunks['where'][] = "$alias.value {$meta_compare} {$where}";
				}
				else
				{
					$sql_chunks['where'][] = "CAST($alias.value AS {$meta_type}) {$meta_compare} {$where}";
				}
			}
		}
	...
	protected function find_compatible_table_alias($clause, $parent_query)
	{
		 ...
		 */
		return apply_filters(WSKLAD_PREFIX . 'data_meta_query_find_compatible_table_alias', $alias, $clause, $parent_query, $this);
	}
